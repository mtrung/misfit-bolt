...
const DEFAULT_ON_RE = /CLTMP 3200/,
      RIGHT_PADDING = /,+$/,
      COLOR_SEPARATOR = ',',
      EMPTY_STRING = '',
      MAX_COLOR = 255,
...
class State {
  ...
  get value() {
    return this._buffer.toString()
      .replace(RIGHT_PADDING, EMPTY_STRING)
      .replace(DEFAULT_ON_RE, [MAX_COLOR, MAX_COLOR, MAX_COLOR].join(COLOR_SEPARATOR));
  }
  ...
  set state(state) {
    debug('set state with state ${state}');
    let alpha;
    if (state) {
  ...
  set color(color) {
    ...
    const rgb = color.rgb(),
          rgba = [rgb.r, rgb.g, rgb.b, this._alpha],
  ...
  set alpha(alpha) {
    valid('alpha', alpha, MAX_ALPHA);
    debug('set alpha with alpha ${alpha}');
    this._alpha = alpha;
    this.color = this.color; // Force color refresh
  }
